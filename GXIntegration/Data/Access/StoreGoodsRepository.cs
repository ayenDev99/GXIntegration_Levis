		...
		public async Task<List<StoreGoodsModel>> GetStoreGoodsAsync(DateTime fromDate, DateTime toDate, string storeCode, string processType)
		{
			using (var connection = new OracleConnection(_connectionString))
			{
				try
				{
					...
					{
						dateCondition = "VOU.POST_DATE BETWEEN :FromDate AND :ToDate";
					}
					else
					{
						dateCondition = "TRUNC(VOU.POST_DATE) BETWEEN :FromDate AND :ToDate";
					...
					string sql = @"
						SELECT
							...
							, TO_NUMBER(PO_ITEM.ITEM_NOTE1) 		AS LineNumber
							...
							, ISI.ALU								AS ALU
							, TO_NUMBER(PO_ITEM.ITEM_NOTE1)			AS ItemLineNumber
				...
				{
					Logger.LogError($"Error fetching Store_Goods data: {ex.Message}");
