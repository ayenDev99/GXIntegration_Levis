			...
			using (var connection = new OracleConnection(_connectionString))
			{
				try
				{
					...
					var sales = await connection.QueryAsync<StoreSaleModel, Items, Discount, Tender, StoreSaleModel>(
						sql,
						(sale, items, disc, tender) =>
						{
							var key = sale.TransSequenceNo;
							...
							if (!salesDictionary.TryGetValue(key, out var existingSale))
							{
								...
								salesDictionary[key] = existingSale;
							}

							if (items != null && !string.IsNullOrEmpty(items?.ItemSequenceNo))
							{
								if (!existingSale.Items.Any(t => t.ItemSequenceNo == items.ItemSequenceNo))
									existingSale.Items.Add(items);
							}

							if (disc != null && !string.IsNullOrEmpty(disc?.DiscSequenceNo))
							{
								if (!existingSale.Discounts.Any(t => t.DiscSequenceNo == disc.DiscSequenceNo))
									existingSale.Discounts.Add(disc);
							}

							if (tender != null && !string.IsNullOrEmpty(tender?.TenderSID))
							{
